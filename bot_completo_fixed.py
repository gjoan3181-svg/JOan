...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║   🎯 BOT ADAPTATIVO v2.1 COMPLETO - CON CORRECCIÓN DE MAPEO                 ║
║                                                                              ║
║   Este es tu código ORIGINAL completo con:                                   ║
║   ✅ Todas las funcionalidades originales                                    ║
║   ✅ Sistema de aprendizaje completo                                         ║
║   ✅ IA como filtro                                                          ║
║   ✅ Todos los indicadores y patrones                                        ║
║   🔧 CORRECCIÓN: Mapeo de datos uno por uno                                  ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class BotAdaptativo:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #                    🔧 MÉTODO CORREGIDO - SOLICITA UNO POR UNO
    # ══════════════════════════════════════════════════════════════════════════
    
    async def obtener_datos(self):
        """
        🔧 CORRECCIÓN: Solicita datos de cada activo UNO POR UNO
        y espera la respuesta antes de solicitar el siguiente.
        Esto garantiza que los datos se asignen al activo correcto.
        """
        ...
        for aid, info in ACTIVOS.items():
            try:
                # Enviar solicitud para ESTE activo
                msg = json.dumps({
                    "name": "sendMessage",
                    "msg": {
                        "name": "get-candles",
                ...
                while intentos < 20:
                    try:
                        ...
                        if data.get('name') == 'candles':
                            ...
                            if candles:
                                self.analizador.agregar_velas(aid, candles)
                                recibidos += 1
                            break
                ...
                if (recibidos + errores) % 10 == 0:
                    print(f"     Progreso: {recibidos + errores}/{total}...")
                    
            except Exception as e:
                errores += 1
        
        print(f"  ✅ {recibidos}/{total} activos cargados correctamente")
        if errores > 0:
            print(f"  ⚠️ {errores} activos no respondieron")
    ...
    def mostrar_menu(self):
        ...
        print("\n  ┌" + "─" * 50 + "┐")
        print(f"  │  📌 BOT ADAPTATIVO + IA {ia_status} (CORREGIDO)" + " " * 10 + "│")
        print("  ├" + "─" * 50 + "┤")
    ...
    async def ejecutar(self):
        limpiar()
        print("\n  🎯 BOT ADAPTATIVO v2.1 COMPLETO - MAPEO CORREGIDO")
        ...
        while True:
            try:
                ...
                if cmd == '1':
                    ...
                    limpiar()
                    print("\n  🎯 BOT ADAPTATIVO v2.1 + IA (CORREGIDO)")
                    print(f"  🕐 {hora_rd()}")
                    self.mostrar_menu()
...
║                                                                    ║
║  🔧 CORRECCIÓN APLICADA:                                           ║
║  • Solicita datos activo por activo (no en batch)                  ║
║  • Garantiza mapeo correcto de datos                               ║
║  • Los precios ahora corresponden al activo correcto               ║
║                                                                    ║
║  ✅ TODAS LAS FUNCIONALIDADES ORIGINALES:                          ║
║  • Sistema de aprendizaje completo                                 ║
║  • IA como filtro inteligente                                      ║
║  • Todos los indicadores y patrones                                ║
║  • Ranking de activos                                              ║
║  • Ajuste automático de parámetros                                 ║
║  • Bloqueo de activos con mal rendimiento                          ║
║                                                                    ║
║  ⚠️ NOTA: La carga inicial es más lenta (~30-60s) pero correcta    ║
║                                                                    ║
╚════════════════════════════════════════════════════════════════════╝
""")
